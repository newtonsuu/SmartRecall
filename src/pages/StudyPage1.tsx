...
function StudyCard({
  ...
  const handleEdit = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    // Navigate to appropriate edit page based on card type
    if (isQuiz) {
      navigate('/edit-quiz/japanese/actual');
    } else {
      navigate('/edit-flashcard/psychology');
    }
  };
  return <div className="bg-white rounded-lg shadow p-4 hover:shadow-md transition-shadow">
      ...
      <div className="flex mt-4 gap-4">
        <button className="text-blue-600 flex items-center gap-1 text-sm" onClick={handleEdit}>
          <EditIcon size={16} />
          <span>Edit</span>
        </button>
...
function SummaryCard({
  ...
  const handleEdit = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    navigate('/edit-summary/japanese');
  };
  return <div className="bg-white rounded-lg shadow p-4 hover:shadow-md transition-shadow">
